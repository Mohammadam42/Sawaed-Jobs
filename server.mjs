...
function buildSeoHead(content, baseUrl) {
  ...
  const description =
    seo.description ||
    "سواعد خدمة احترافية تساعد الباحثين عن عمل على العثور على وظائف مناسبة وتنظيم التقديم والمتابعة واستلام تقارير واضحة.";
  const title = seo.title || "سواعد | طريقك لوظيفة العمر";
  const keywords = seo.keywords || "سواعد, تقديم وظائف, متابعة وظائف, وظائف الأردن, فرص عمل, سيرة ذاتية";
  const image = `${baseUrl}/assets/sawaed-process-v2.png`;
  const instagramUrl = String(brand.instagramUrl || "").trim();
  const jsonLd = {
  ...
  if (instagramUrl && brand.instagramEnabled !== false) {
...
if (isProduction) {
  ...
  app.get("*", async (req, res, next) => {
    try {
      ...
      const title = escapeHtml(content.seo?.title || "سواعد | طريقك لوظيفة العمر");
